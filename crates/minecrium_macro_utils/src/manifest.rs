...
fn manifest() -> &'static Document {
    static MANIFEST: OnceCell<Document> = OnceCell::new();

    MANIFEST.get_or_init(|| match std::env::var_os("CARGO_MANIFEST_DIR") {
        Some(path) => {
...
pub fn get_crate_path(name: &str) -> Path {
    ...
    } else if let Some(deps) = manifest().get("dev-dependencies") {
        if let Some(path) = find_in_deps(name, deps) {
            return path;
        }
    }
